...
df = pd.DataFrame(columns=['Feature', 'Max correlation Feature', 'correlation value'])
...
for column in pearson_corr.columns:
    ...
    df = df.append(pd.DataFrame({'Feature': [column], 'Max correlation Feature': [filtered_frame.index[0]], 'correlation value':[str(filtered_frame[column][0])]}), sort=False)
